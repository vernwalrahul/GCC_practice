...
int calculate_cost(vector<A> &objects_sorted, int n, int B)
{
	...
	while(i<=B){
			...
			// cout<<"initially max_cost="<<max_cost<<endl;
			for(int j=1;j<n;j++) {
			
				if(i- objects_sorted[j].w>=0) {
					if(objects_sorted[j].c+cost[i- objects_sorted[j].w]> max_cost) {
						max_cost=objects_sorted[j].c+cost[i- objects_sorted[j].w]; index=j;
					}
				}
				else
					break;
...
int main()
{
	...
	int maxm_cost=calculate_cost(objects_sorted, n, B);
